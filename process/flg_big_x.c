...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/01/04 10:44:48 by igearhea          #+#    #+#             */
/*   Updated: 2021/01/04 10:44:50 by igearhea         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		flg_big_x_print_right(t_printf_list *printf_list, char *str,
																		int len)
{
	int	i;
...
void		flg_big_x_print_left(t_printf_list *printf_list, char *str, int len)
{
	int	i;
...
void		flg_big_x(t_printf_list *printf_list)
{
	...
	num = va_arg(printf_list->arg_pointer, unsigned int);
	minus = 0;
	if (num < 0)
		minus = 1;
	str = get_big_hex_string(num);
	len = ft_strlen(str);
	...
	if ((num == 0) && (printf_list->no_precision == 0) &&
		...
		flg_big_x_print_left(printf_list, str_after_prec, len);
	else
		flg_big_x_print_right(printf_list, str_after_prec, len);
	free(str_after_prec);
}
